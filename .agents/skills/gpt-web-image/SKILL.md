...
7. 每次实际网页提交前自动选择模型：`GPT-5.6 Sol 高`、`GPT-5.6 Sol 中`、`极速`。只能确认网页明确可用且最终选中状态已验证的模型；三者均明确不可用时只返回 `单日生图已达限额，暂时不可生图。`，菜单结构不明确时报告模型选择不确定。
...
10. `MODEL_SELECTION_UNCERTAIN` 或提交前的模型控件点击超时表示网页结构无法确认；不得把它归因于登录失效或额度耗尽，也不得盲目重复同一提交。只有 CLI 明确返回 `LOGIN_REQUIRED`、`HUMAN_VERIFICATION_REQUIRED` 或单日限额文案时，才能按对应状态提示用户。
