...
当前 0.2.5 代码提交 `1f603767c863ceef88a8c8ee742e730920277491` 对应 run `30816483993` / job `91695409500` 全步骤成功，覆盖安装依赖、Google Chrome 探测、typecheck、全量合同/安全/安装/浏览器夹具测试和 build。该结果收口当前 T39、MP-7 和 IMG-6，但不等同 Windows x64 用户真机验收。公开 Actions 日志下载接口返回 403，本次以 API 返回的 job/step 完整成功状态为证据，不声称已读取日志正文。
